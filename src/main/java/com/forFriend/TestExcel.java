...
public class TestExcel {
    ...
    public static void main(String[] args) {
        String inFileName = "D:" + File.separator + "aa.xlsx";
        File file = new File(inFileName);
        ArrayList<ArrayList<Object>> result = TestExcel.readExcel(file);
        for(int i = 0 ;i < result.size() ;i++){
            for(int j = 0;j<result.get(i).size(); j++){
                System.out.println(i+"行 "+j+"列  "+ result.get(i).get(j).toString());
            }
        }
        String outFileName = "D:" + File.separator + "bb.xlsx";
        TestExcel.writeExcel(result,outFileName);
    }
